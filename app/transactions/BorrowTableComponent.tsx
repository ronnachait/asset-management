...
import ExtendOrder from "./Extend";
type Asset = {
  id: string;
  asset_number: string;
  asset_name: string;
...
export function BorrowOrderTableComponent() {
  ...
  const fetchOrders = async () => {
    const res = await fetch("/api/borrow-orders");
        ...
        <table className="w-full table-fixed divide-y divide-gray-200 text-sm">
          <thead className="bg-blue-600 text-white text-xs uppercase tracking-wider dark:bg-gray-700">
            <tr>
              ...
              <th className="px-4 py-3 text-center">วันที่ยืม</th>
              <th className="px-4 py-3 text-center">กำหนดคืน</th>
              <th className="px-4 py-3 text-center">วันคงเหลือ</th>
              <th className="px-4 py-3 text-center">วันที่คืน</th>
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-center w-36">
                        <div className="flex flex-wrap justify-center items-center gap-2">
                          <button
                            aria-label={`ดูรายละเอียดคำสั่งที่ ${startIndex + index + 1}`}
                            className="cursor-pointer bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded-2xl text-sm font-medium transition-colors duration-200 shadow-sm hover:shadow-md whitespace-nowrap"
